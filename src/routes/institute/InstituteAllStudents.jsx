...
const InstutueStudents = () => {
    ...
    useEffect(() => {
        ...
        fetchDefaultOptions();
        getAllUsers();
    }, [_id, limit, navigate, page, searchQuery, sortType]);

    const pageSizeOptions = [5, 10, 20, 50, 100, 7];
    function handleInstitutesClick() {
        navigate(`/user/register/${_id}`);
    }
    ...
    const loadOptions = async (inputValue, callback) => {
        try {
            const response = await axios.get(`${SERVER_ORIGIN}/api/institute/auth/institute/all?id=${_id}`, {
                params: {
                    search: inputValue,
                    page: 1,
                    limit: 10,
